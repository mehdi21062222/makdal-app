  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,color:C.black,fontFamily:"'Plus Jakarta Sans',sans-serif",paddingBottom:90}}>
        ...
        {page==="personnel" && <div className="fade">
          ...
          {activePersoTab==="equipes" && <>
            ...
            {employees.filter(e=>e.team===activeTeam).map(emp=>{
              ...
              const allAvEmp = avances.filter(a=>a.empId===String(emp.id)).sort((a,b)=>b.date.localeCompare(a.date));
          ...
          {activePersoTab==="avances" && <>
            ...
            {employees.map(emp=>{
              const empAvances = avances.filter(a=>String(a.empId)===String(emp.id)).sort((a,b)=>b.date.localeCompare(a.date));
              if (empAvances.length===0) return null;
              const totalNonRemb = empAvances.filter(a=>!a.rembourse).reduce((s,a)=>s+Number(a.montant),0);
              return (
                <div key={emp.id} className="card" style={{marginBottom:10}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                    <div style={{display:"flex",alignItems:"center",gap:8}}>
                      <span style={{fontSize:16}}>{ROLE_ICON[emp.role]||"👤"}</span>
                      <span style={{fontWeight:700,fontSize:14}}>{emp.name}</span>
                    </div>
                    <div style={{fontWeight:800,color:totalNonRemb>0?C.red:C.gray,fontSize:14}}>
                      {totalNonRemb>0 ? `${totalNonRemb.toLocaleString()} DH` : "Tout remboursé ✓"}
                    </div>
                  </div>
                  <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                    {empAvances.map(av=>{
                      const [y,m,d] = av.date.split("-");
                      return (
                        <button key={av.id} onClick={()=>{setAvanceForm({...av});setAvanceModal(av);}} style={{
                          display:"flex",alignItems:"center",gap:5,
                          background: av.rembourse ? C.grayLight : C.magentaPale,
                          border:`1px solid ${av.rembourse?C.border:resto.color+"55"}`,
                          borderRadius:20,padding:"6px 11px",fontSize:11.5,fontWeight:700,
                          color: av.rembourse ? C.gray : C.magentaDark,
                          cursor:"pointer", fontFamily:"inherit",
                        }}>
                          <span style={{opacity:.75}}>{d}/{m}</span>
                          <span style={{textDecoration:av.rembourse?"line-through":"none"}}>{Number(av.montant).toLocaleString()} DH</span>
                          {av.rembourse && <span style={{fontSize:10}}>✓</span>}
                        </button>
                      );
                    })}
                  </div>
            ...
            {avances.length>0 && employees.every(emp=>avances.filter(a=>String(a.empId)===String(emp.id)).length===0) && (
              <div className="card" style={{textAlign:"center",padding:"20px",color:C.gray,fontSize:13}}>
                Des avances existent mais ne correspondent à aucun employé actuel.
              </div>
            )}
          </>}
        </div>}
      ...
      {avanceModal && <Modal title={avanceModal==="add"?"Nouvelle avance":"Modifier"} onClose={()=>setAvanceModal(null)}>
        ...
        {avanceModal!=="add" && (
          <button onClick={()=>{toggleRembourse(avanceModal.id);setAvanceForm(p=>({...p,rembourse:!p.rembourse}));notify(avanceForm.rembourse?"Réactivée":"Remboursée ✓");}} style={{
            width:"100%",display:"flex",alignItems:"center",justifyContent:"center",gap:8,
            background: avanceForm.rembourse ? `${C.green}18` : C.grayLight,
            color: avanceForm.rembourse ? C.green : C.gray,
            border:"none",borderRadius:12,padding:"12px 0",fontWeight:700,fontSize:13,cursor:"pointer",marginBottom:14,
          }}>
            {avanceForm.rembourse ? "✓ Marquée comme remboursée (tap pour annuler)" : "Marquer comme remboursée"}
          </button>
        )}
        <div style={{display:"flex",gap:8,marginTop:6}}>
